...
export const requireSignIn = async (req,res,next)=>{
  try {
    const decode = JWT.verify(req.headers.authorization, process.env.JWT_SECRET); //incrypt
...
try{
  let dd = req.body;
  console.log("DAta in Braintree", req.body);
  const productd = await packagesModel.findById({_id : req.body.booking.package}).select("destinationName activity duration price ")
    console.log(productd)

    const user = await userModel.findOne({email: req.body.booking.user.email }).select("id name email")
    // console.log("USerId: ",userId)
    const order = new orderModel({
      product: productd._id,
      payment: "Success",
      buyer: user._id,
      bookingOn: req.body.booking.bookingOn
    })
    await order.save();
    // toast.success("Order Placed Succesfully");
    // console.log("Order Placed Succesfully")
